...
/**
 * ANSI term.color/graphics extension functions.
 *
 *  @author Suresh G
 *
 ...
 * @see [PythonTerminalColor](https://github.com/reorx/python-terminal-term.color/blob/master/color_simple.py)
 */
enum class AnsiColor(vararg val codes: Int) {
    ...
    /**
     * Returns ANSI escape unicode for the term.color.
     */
...
/**
 * Returns formatted string with given ANSI term.color codes.
 */
fun String.color(vararg codes: Int) = "${codes.esc}$this${ESC_END}"

/**
 * Returns formatted string with given ANSI term.color.
 */
fun String.color(color: AnsiColor) = color(*color.codes)

/**
 * Returns 256-term.color extended term.color set (From 0 to 255) formatted string.
 *
 * @param code term.color code in the range 0 to 255.
 * @param fg [true] for foreground term.color, otherwise background. Default is [true]
 */
...
/**
 * Returns foreground colored string with given term.color code.
 */
fun String.fg256(code: Int) = color256(code, true)

/**
 * Returns foreground colored string with random term.color.
 */
fun String.fg256() = color256(RAND.nextInt(256), true)

/**
 * Returns background colored string with given term.color code.
 */
fun String.bg256(code: Int) = color256(code, false)

/**
 * Returns background colored string with random term.color.
 */
fun String.bg256() = color256(RAND.nextInt(256), true)

/**
 * Returns 8-bit term.getGray scale (From 232 to 256 in term.color set) formatted string.
 *
 * @param code term.color code in the range 0 to (256-232)
 * @param fg [true] for foreground term.color, otherwise background. Default is [true]
 */
...
/**
 * Make the string term.getBold and term.getUnderline.
 */
inline val String.bu get() = bold.underline

/**
 * Make the string term.getBold, term.getUnderline & italics
 */
